...
if(null==$text)return"";if(void 0==$text)return"";const lCurrentLocale=$locales._current_lang_;if("object"==typeof $text)if(void 0!=$text[lCurrentLocale])return $text[lCurrentLocale];else return $text[Object.keys($text)[0]];return $text.toString().translate()}}),siApp.filter("siElementExists",function(){return function($element){return document.querySelectorAll($element).length>0}});const findProperty=function($source,$path){let lPathParts=$path.split("."),lCursor=$source,lResult=null;return lPathParts.forEach(function($e){if("undefined"!=typeof lCursor[$e])lCursor=lCursor[$e],lResult=lCursor;else lResult=null}),lResult};