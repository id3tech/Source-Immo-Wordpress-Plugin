...
if(isset($configs) && $configs !== false){
    ...
    $configs = new SourceImmoList('','offices','offices','',['address','city']);
    //$configs->type = 'listings';
    //$configs->list_item_layout->displayed_vars->main = ['address','city','price','category','subcategory','rooms'];
}
?>
<article class="si-item si-office-item  si-single-layer-item-layout style-standard si-border <?php echo($scope_class) ?> {{getClassList(item)}}"  
    data-ng-cloak>
